  ...
  if (!(rootContainerElement: any)[listeningMarker]) {
    ...
    if (ownerDocument !== null) {
      ...
      if (!(ownerDocument: any)[listeningMarker]) {
        ...
        //> 将 selectionchange 绑定到 document
...
function addTrappedEventListener(
  ...
  // 获取对应事件，事件定义在 ReactDOMEventListener.js
  //> eg: DiscreteEventPriority 对应 dispatchDiscreteEvent
  //>     ContinuousEventPriority 对应 dispatchContinuousEvent
  ...
  if (passiveBrowserEventsSupported) {
    ...
    //> React 不再将他们绑定到 document 上，但是现在改变这一点将会撤销之前的性能优势。
    ...
    if (
      ...
      isPassiveListener = true; //? 对于这三种事件模拟实现 passive
    }
  }

  //* 17 之后，事件委托在 targetContainer，在兼容之前的版本委托在 document
  targetContainer =
  ...
  if (isCapturePhaseListener) {
    ...
    if (isPassiveListener !== undefined) {
      //* 对 touchstart touchmove wheel 添加事件捕获监听器，并设置 passive 标志
      ...
      //* 对其他事件添加事件捕获监听器
      //> click、contextmenu、drag、drop、input、mousedown、change 等
