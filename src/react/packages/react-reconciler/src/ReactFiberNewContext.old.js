...
function propagateContextChange_eager<T>(
  ...
  while (fiber !== null) {
    ...
    // DPS: 遍历当前fiber的依赖
    const list = fiber.dependencies; // fiber 的 dependencies 是一个 ContextDependency 链表
    if (list !== null) {
      nextFiber = fiber.child;

      let dependency = list.firstContext;
      //> 如果有 context 消费，则遍历 contextDependency 链表，找到匹配的 context 并标记 fiber lanes
    ...
    if (enableLazyContextPropagation) {
      ...
      if (firstContext !== null) {
        if (includesSomeLane(dependencies.lanes, renderLanes)) {
          //> 如果当前fiber的dependencies.lanes包含renderLanes，则标记当前fiber接收到了更新
...
export function readContext<T>(context: ReactContext<T>): T {
  ...
  if (lastFullyObservedContext === context) {
    //> 如果lastFullyObservedContext与context相同，则说明当前fiber已经观察过context的更新，则不需要进行任何操作
    ...
    if (lastContextDependency === null) {
      //> 如果lastContextDependency为null，则说明当前fiber没有依赖任何context
      ...
      //> 此时更新 lastContextDependency 为 contextItem 做为第一个依赖
      ...
      //> 如果lastContextDependency不为null，则说明当前fiber有依赖其他context，
      //   则将contextItem添加到lastContextDependency的next
