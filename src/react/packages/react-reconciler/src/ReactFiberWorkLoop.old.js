...
export function requestUpdateLane(fiber: Fiber): Lane {
  ...
  // !React 内部的一些更新，eg：flushSync 会通过上下文变量跟踪其优先级
  ...
  // !React 外部的 update，根据事件类型 想当前环境获取对应的优先级
...
//! 页面初次渲染、类组件 setState/forceUpdate、函数组件的 setState/useEffect 等都会调用此函数
export function scheduleUpdateOnFiber(
  ...
  // Mark that the root has a pending update. 待处理的更新
  ...
  if (
    ...
    //* 在开发环境下，如果更新是在渲染阶段触发的，则警告
    //* 如果更新来自用户空间（除了本地 hook 更新，这些更新会被单独处理，不会到达这里），那么这是一个错误，则警告
    //* 但是，某些内部 React 特性（如选择性水合）使用此作为实现细节，因此不会警告
    ...
    //* 非紧急更新等常规逻辑 >>>
    //* 页面初次渲染时，此时的 workInProgressRoot 为 null
...
//! render 阶段开始：任务调度器
//! 执行并发任务的入口，即所有通过 Scheduler 执行的任务
//* 包含了两个重要阶段：render 和 commit
function performConcurrentWorkOnRoot(root, didTimeout) {
  ...
  //! 某些情况下，会禁用时间切片：如果 work 长时间做计算（防止饥饿而将其视为“过期”work）或者处于默认启用同步渲染
  ...
  const shouldTimeSlice =
    ...
    //? 校验是否过期
    !includesExpiredLane(root, lanes) && //* 参考：UseDeferredValuePage
    (disableSchedulerTimeoutInWorkLoop || !didTimeout);

  //! 1. render: 根据是否启用时间切片，执行不同的渲染方式
  let exitStatus = shouldTimeSlice
    ? renderRootConcurrent(root, lanes) //* 参考：UseDeferredValuePage
    : renderRootSync(root, lanes); //! 不用时间切片，同步渲染
  ...
  if (exitStatus !== RootInProgress) {
    ...
    if (exitStatus === RootDidNotComplete) {
      ...
      //! 2. 如果 render 阶段完成，接下来进入 commit 阶段
      ...
      //! 3. commit
      //* 现在得到了一个一致的树，接下来要么提交它，要么等待一个超时之后再提交
...
function prepareFreshStack(root: FiberRoot, lanes: Lanes): Fiber {
  //! 1. 因为是刚刚开始 render 阶段，所以重置 root 的 finishedWork 和 finishedLanes
  ...
  //! 2. 重置栈
  //* 重置栈的目的是为了确保在 render 阶段开始时，栈是空的
  //* 把 concurrentQueue 中的任务转移到 fiber 的 updateQueue 中，即给 fiber 的 lanes、childLanes 赋值
  finishQueueingConcurrentUpdates();
...
function renderRootSync(root: FiberRoot, lanes: Lanes) {
  ...
  //! 重置 Context 相关值，如 currentRenderingFiber 等
  resetContextDependencies();
  // 重置 executionContext
  executionContext = prevExecutionContext;
...
function workLoopSync() {
  //! 1. 同步渲染，一直执行到 workInProgress 为 null
...
function performUnitOfWork(unitOfWork: Fiber): void {
  ...
  if (enableProfilerTimer && (unitOfWork.mode & ProfileMode) !== NoMode) {
    ...
    //! 更新当前 fiber，包括 props、state、context，生命周期函数的执行、Hooks 的执行等
    //! 返回值 子节点 fiber 的处理：新的 workInProgress > next fiber
  ...
  resetCurrentDebugFiberInDEV();
  //* 把 unitOfWork 的 pendingProps 更新到 memoizedProps
  unitOfWork.memoizedProps = unitOfWork.pendingProps;
  if (next === null) {
    // If this doesn't spawn new work, complete the current work.
    //* 如果 beginWork 没有生成新的 work，则完成当前的 work > 意味着结束
    completeUnitOfWork(unitOfWork);
  } else {
    // workInProgress 中的条件则可以继续执行
    workInProgress = next;
  }
  //* 重置 ReactCurrentOwner.current
  ReactCurrentOwner.current = null;
}
...
function commitRootImpl(
  ...
  // They're redundant.
  //* useEffect 的 setup 回调异步执行的触发位置
  if (
    (finishedWork.subtreeFlags & PassiveMask) !== NoFlags ||
    (finishedWork.flags & PassiveMask) !== NoFlags
  ) {
    //* 如果存在 passive effects，则调度一个回调来处理它们
    if (!rootDoesHavePassiveEffects) {
      //* 如果还没有处理过 passive effects，则设置标志
      console.log(
        `🧠 \x1b[91mFile: ReactFiberWorkLoop.old.js\x1b[0m, \x1b[32mLine: 2136\x1b[0m, Message: `,
        'flushPassiveEffects',
      );
      ...
      //! 1. 通过异步调度回调异步处理 passive effects
      //? hook 函数组件的 useEffect、useLayoutEffect 的 setup 回调异步执行的触发位置
      //?      这不同于类组件的 componentDidMount、componentDidUpdate 在 commit 阶段同步执行
      //? 虽然我们在 useEffect 和 didMount 中做类似的逻辑，但是两者实际上是不对等的 执行时间就不一样
      scheduleCallback(NormalSchedulerPriority, () => {
        flushPassiveEffects(); //> 执行异步处理
  ...
  if (subtreeHasEffects || rootHasEffect) {
    ...
    //! 4. layout 阶段
    //? 类组件的 didMount 和 didUpdate 在此阶段：在 dom 变更后同步执行的，相对函数组件的 useEffect、useLayoutEffect 稍晚执行
...
export function flushPassiveEffects(): boolean {
  ...
  if (rootWithPendingPassiveEffects !== null) {
    //* 如果存在 pending passive effects，则进行处理
