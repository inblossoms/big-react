...
//! 无论是 layoutEffect 或是 passiveEffect 调用的都是该函数
  ...
  if (lastEffect !== null) {
    ...
    do {
      if ((effect.tag & flags) === flags) {
        ...
        //* Mount: unmount 执行的是 destroy，这里是 create 函数
        ...
        if (finishedWork.flags & Update) {
          if (!offscreenSubtreeWasHidden) {
            if (current === null) {
              ...
              //? current !== null 即存在上一次的 fiber 节点，说明是 update 阶段 需要处理更新逻辑
              //* update 阶段，获取上一次的 props 和 state
              const prevProps =
                finishedWork.elementType === finishedWork.type
    ...
    findParent: while (parent !== null) {
      switch (parent.tag) {
        case HostComponent: {
          ...
          break findParent; //? 类组件中可能存在多个生命周期函数处理，所以需要遍历
        }
        case HostRoot: {
  ...
  switch (deletedFiber.tag) {
      ...
      if (!offscreenSubtreeWasHidden) {
        ...
        if (updateQueue !== null) {
          ...
          if (lastEffect !== null) {
            ...
            let effect = firstEffect;
            do {
              //> 函数组件如果存在 destroy(cleanup) 函数，需要遍历检查执行
    ...
    case ClassComponent: {
      if (!offscreenSubtreeWasHidden) {
        ...
        //> 类组件中如果存在 componentWillUnmount 函数，需要执行
