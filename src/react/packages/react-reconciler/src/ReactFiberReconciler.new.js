...
export function createContainer(
  ...
  return createFiberRoot(
    containerInfo, // 根 dom 节点： <div id="root"></div>
    tag, // 根节点类型： RootTag.Root
    hydrate, // 是否进行服务端渲染
    initialChildren, // 初始子节点
    hydrationCallbacks, // 水合回调
    isStrictMode, // 是否开启严格模式
    concurrentUpdatesByDefaultOverride, // 并发更新
...
// updateContainer(chidren, root, null, null)
export function updateContainer(
  element: ReactNodeList, // 子节点：要渲染的元素
  container: OpaqueRoot, // 根节点：根 fiber 节点
  // 从 18 版本开始下面两个参数传入 null (为了兼容旧版本代码)
  ...
  //! 1. 获取根节点 fiber > current、lane
  const current = container.current;
  const eventTime = requestEventTime();
  // lane 用于标识 update 优先级，可以理解为表示 update 的优先级的一种机制
  // 每个 update 会被分配一个或者多个 lane 以确定其在更新队列中的优先级顺序
  ...
  // parentComponent 为 null，此处只返回一个空对象 用于兼容旧版本
