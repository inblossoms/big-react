...
export function renderWithHooks<Props, SecondArg>(
  ...
  if (__DEV__) {
    ...
    //> 根据当前 fiber 是否存在，以及是否存在 memoizedState 来决定使用哪个 dispatcher
...
function mountWorkInProgressHook(): Hook {
  ...
  //? workInProgressHook: 当前组件的 fiber 的 memoizedState 链表的头部
  //* hooks 是一个链表
...
function updateWorkInProgressHook(): Hook {
  ...
  if (currentHook === null) {
    //> currentHook 就是当前 workInProgressHook 所对应的 fiber 的 memoizedState 链表的头部
    const current = currentlyRenderingFiber.alternate;
    if (current !== null) {
      //> 进入此判断则意味着有老的 fiber 节点，可以复用老的 fiber 节点的 memoizedState 链表
  ...
  // 函数组件刚进入更新阶段，workInProgressHook 为 null
  if (workInProgressHook === null) {
    nextWorkInProgressHook = currentlyRenderingFiber.memoizedState;
  } else {
    // 否则，从当前组件的 fiber 的 memoizedState 链表中获取下一个 hook
    nextWorkInProgressHook = workInProgressHook.next;
  }

  if (nextWorkInProgressHook !== null) {
    //> 如果 nextWorkInProgressHook 不为 null，则意味着有老的 hook 可以复用
...
function mountReducer<S, I, A>(
  ...
  const hook = mountWorkInProgressHook(); // 将 hook 添加到当前组件的 fiber 上
  let initialState;
  if (init !== undefined) {
  ...
  const queue: UpdateQueue<S, A> = {
    ...
    lastRenderedState: (initialState: any), // 当前 hook 的 memoizedState，上一次的状态值
  };
  hook.queue = queue;
...
//! useState 和 useReducer 在组件更新阶段走的都是 updateReducer 函数
function updateReducer<S, I, A>(
  reducer: (S, A) => S,
  initialArg: I,
  init?: I => S,
): [S, Dispatch<A>] {
  const hook = updateWorkInProgressHook(); // 更新阶段，获取当前新 hook 数据
  const queue = hook.queue;
  ...
  //> 更新 queue 的 reducer > 记录 reducer
  queue.lastRenderedReducer = reducer;

  const current: Hook = (currentHook: any); // 当前 hook 数据 > 旧数据

  // The last rebase update that is NOT part of the base state.
  let baseQueue = current.baseQueue; //> 单向循环链表
  ...
  if (pendingQueue !== null) {
    ...
    //? 置空 避免下一次渲染时，因为存在数据而导致重复计算
    queue.pending = null;
  }

  if (baseQueue !== null) {
    // We have a queue to process.
    const first = baseQueue.next; //! 通过 baseQueue 尾节点的 next 获取到第一个 update
    ...
    do {
      ...
      update = update.next;
    } while (update !== null && update !== first); //> 当 update 和 first 相等时，说明已经遍历完所有的 update
    ...
    //? 更新 hook 数据
    hook.memoizedState = newState; //> 更新后的 state 会先存储到 hook 上，hook 最终会存储到 fiber 的 memoizedState 链表上
...
//! 这里更新 state 和 useReducer 的原理是一样的，都是通过 updateReducer 函数来更新 state
...
function dispatchReducerAction<S, A>(
  ...
  const lane = requestUpdateLane(fiber);
  //? 1. 创建一个 update 对象，用于存储当前的 action 和 lane
  ...
  //? 2. 如果当前 fiber 正在渲染，则将 update 添加到渲染队列中
  if (isRenderPhaseUpdate(fiber)) {
    ...
    if (root !== null) {
      const eventTime = requestEventTime();
      //? 3. 如果 root 不为空，则将 update 添加到渲染队列中，等待调度
...
function dispatchSetState<S, A>(
  ...
  if (isRenderPhaseUpdate(fiber)) {
    ...
    if (
      ...
      //? 3. 如果当前 fiber 的 lanes 和 alternate 的 lanes 都为 NoLanes，则意味着当前 fiber 的 lanes 和 alternate 的 lanes 都为空，可以快速计算出新的状态值
      ...
      if (lastRenderedReducer !== null) {
        ...
        try {
          ...
          if (is(eagerState, currentState)) {
            ...
            //* 如果 state 没有变化，则组件不做更新
            //! 这里和 useReducer 不同，useReducer 在 state 没有变化时，会重新渲染组件
    ...
    //? 4. 将 update 添加到渲染队列中 > 将 update 添加到 fiber 的 updateQueue 中
    const root = enqueueConcurrentHookUpdate(fiber, queue, update, lane);
    if (root !== null) {
      const eventTime = requestEventTime();
      //? 5. 如果 root 不为空，则将 update 添加到渲染队列中，等待调度
      scheduleUpdateOnFiber(root, fiber, lane, eventTime);
      entangleTransitionUpdate(root, queue, lane);
    }
  }
