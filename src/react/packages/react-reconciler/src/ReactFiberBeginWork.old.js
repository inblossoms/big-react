...
function updateFunctionComponent(
  ...
  if (__DEV__) {
    ...
    //? renderWithHooks 将函数组件内部的所使用到的 hook 关联到当前组件的 fiber 上
    nextChildren = renderWithHooks(
      current,
      workInProgress,
      Component,
...
function updateHostRoot(current, workInProgress, renderLanes) {
  //   console.log('updateHostRoot: mark');

  pushHostRootContext(workInProgress);

  //* current 是根 fiber，current.stateNode > FiberRoot
  if (current === null) {
    throw new Error('Should have a current fiber. This is a bug in React.');
  }

  //! 1. 更新 props、state、updateQueue、transtion
  const nextProps = workInProgress.pendingProps;
  const prevState = workInProgress.memoizedState;
  const prevChildren = prevState.element;
  //* 克隆 updateQueue 到 workInProgress
  cloneUpdateQueue(current, workInProgress);
  processUpdateQueue(workInProgress, nextProps, null, renderLanes);

  //! 2. 更新，经过 processUpdateQueue 后，数据可能变化，需要重新赋值
  ...
  if (supportsHydration && prevState.isDehydrated) {
    ...
    // already hydrated.
    //! 3. bailout or 协调子节点
    //* bailout 阶段返回 null。不会发生在组件初次渲染阶段，仅仅发生在组件更新阶段。
    //* 当组件子节点没有发生变化，或者开发人员使用 React.memo 包裹组件，或者调用 shouldComponentUpdate 等
    //* 组件子节点不需要协调时，那么就会发生 bailout。
    resetHydrationState();
...
function updateHostComponent(
  ...
  //? 如果只有一个文本，便不做为 chidren 而是做为当前节点属性进行更新
  ...
  if (enableLazyContextPropagation) {
    ...
    if (oldProps !== null) {
      ...
      if (is(oldValue, newValue)) {
        ...
        //* 如果 旧的 children 和 新的 children 相同，并且没有上下文变化，则跳过更新
        //! 根本原因是因为协调节点是一个昂贵的操作
        if (
          oldProps.children === newProps.children &&
          !hasLegacyContextChanged() // 老版本中的 context
        ) {
          return bailoutOnAlreadyFinishedWork(
            current,
            workInProgress,
        ...
        //* context value 已更改，搜索匹配的消费者并调度他们进行更新
        // them to update.
...
function bailoutOnAlreadyFinishedWork(
  ...
  if (!includesSomeLane(renderLanes, workInProgress.childLanes)) {
    //* 1. 子节点没有 pending work 跳过返回  null
  ...
  // fibers and continue.
  //* 2. 当前 fiber 没有 pending work，但子节点有。此时 clone child fibers 返回 child
...
function beginWork(
  ...
  //? current 是旧 fiber，所以初次渲染不会走这个逻辑
  if (current !== null) {
    //* 更新阶段
  ...
  // ! 在进行开始阶段之前，清除待处理的更新优先级
  // todo: 这里假设即将要评估组件并处理更新队列，然而一个例外情况：SimpleMemoComponent
  //       有时会在开始阶段后退出。这意味着应该将这个赋值移除常规路径，放入到各个分支
